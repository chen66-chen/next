...
async function updateArticleContent() {
  ...
  // 在这里定义文章内容，使用更安全的方式处理特殊字符
  const articleContents = {
    'linux-basic-guide': `# Linux基础入门指南
...
在选择文件系统时，应根据具体应用场景、性能需求和数据安全要求进行评估。`,
    ...
    'linux-shell-scripting': `# Linux Shell脚本编程精通
...
Shell脚本是Linux系统管理和自动化的强大工具，掌握它可以大大提高工作效率。`,
    ...
    'network-security-learning': `# 网络安全学习笔记
...
今天的学习非常充实，期待明天继续深入网络安全领域的学习！`,
    ...
    'markdown-quick-start': `# Markdown快速入门指南
...
希望这份快速入门指南对你有所帮助！随着实践，你会发现Markdown是一种非常高效的文档编写方式。`,
    ...
    'ai-attention-mechanism': `# AI研究笔记: 注意力机制
...
*注：本笔记基于我对注意力机制的理解和研究，仅供学习参考。*`
  ...
  try {
    ...
    const posts = await prisma.post.findMany({
      where: {
        slug: {
          in: Object.keys(articleContents)
        }
      }
    });
    ...
    for (const post of posts) {
      const richContent = articleContents[post.slug];
      ...
      if (richContent) {
        ...
        console.log(`更新了文章内容: ${post.title}`);
        updateCount++;
      }
    }
    
    console.log(`\n完成! 已更新 ${updateCount} 篇文章的内容`);
